...
void AccountFile::RegisterAccount(const char *UserId, const char *password, const char *name) {  //添加一个新账户
...
void AccountFile::ChangePassword(const char *UserId, const char *new_password, LogStatus &log_status,const char *current_password) {
...
void AccountFile::CreatAccount(const char *UserId, const char *password,const int priority, const char *name,  LogStatus &log_status) {
...
void AccountFile::DeleteAccount(const char *UserId,LogStatus &log_status) {
    std::vector<int> user_num_vec=id_num.FindInBlock(UserId);
